...
function buildInsights(){
 ...
 const speakers=data.nodes.filter(n=>n.speech?.quotes>=30).sort((a,b)=>b.speech.kindnessScore-a.speech.kindnessScore);
 const kindest=speakers[0];
 document.querySelector("#kindness-title").textContent=`${kindest.id} has the most positive attributed dialogue.`;
 document.querySelector("#kindness-copy").textContent=`Across ${kindest.speech.quotes} explicitly attributed quotes (${kindest.speech.words.toLocaleString()} words), the mean VADER score is ${kindest.speech.meanSentiment}. A 30-quote minimum reduces small-sample winners; this estimates language positivity, not moral character.`;
 document.querySelector("#kindness-score").innerHTML=`${kindest.speech.kindnessScore}<small> / 100</small>`;
